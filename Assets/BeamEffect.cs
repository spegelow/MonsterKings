...
//Based on Hovl_Laser2 by Hovl Studios
public class BeamEffect : MonoBehaviour
{
    ...
    //SVP Used to control the beam through states instead of hit detection
    public BeamAttack attack;

    public void Initialize(BeamAttack attack)
    {
    ...
    void FixedUpdate()
    {
        if (laserPS != null && UpdateSaver == false)
        {
            ...
            //rotate the beam if needed
            this.transform.rotation = Quaternion.LookRotation(attack.transform.position - transform.position);
            
            var EndPos = attack.transform.position;
            ...
            if (attack.beamState == BeamAttack.BeamState.HitTarget || attack.beamState == BeamAttack.BeamState.BeamClash)
            {
                if (Hit != null)
                {
                    //TODO, add directionality to hit effect if needed
            ...
            else if(attack.beamState == BeamAttack.BeamState.NoHits)
            {
